...
class MoveTurtle(Node):  # 사각형을 그리는 노드 클래스 정의
    def __init__(self):
        super().__init__('move_turtle')  # ROS 2 노드 이름을 'move_turtle'로 설정
        
        # 파라미터 선언
        self.declare_parameter('linear_velocity', 1.0)  # 선속도 (기본값: 1.0)
        self.declare_parameter('angular_velocity', 1.0)  # 각속도 (기본값: 1.0)
        self.declare_parameter('distance', 2.0)  # 직선 이동 거리 (기본값: 2.0)
        self.declare_parameter('angle', math.pi / 2)  # 회전 각도 (기본값: 90도)
        ...
        self.start_y = 0.0  # 직진 시작 지점 y 좌표
        self.start_theta = 0.0  # 회전 시작 방향 (라디안)
        self.distance = self.get_parameter('distance').get_parameter_value().double_value  # 각 직선 구간에서 이동할 거리
        self.angle = self.get_parameter('angle').get_parameter_value().double_value  # 회전할 각도 (90도 = π/2 라디안)
        self.turn_count = 0  # 현재까지 몇 번 꺾었는지 (4번이면 사각형 완성)
    ...
    def timer_callback(self):
        ...
        if self.state == 'forward':
            ...
            if dist_moved < self.distance:
                msg.linear.x = linear_velocity  # 아직 목표 거리 못 갔으면 직진 유지
            else:
                msg.linear.x = 0.0  # 거리 도달 시 멈춤
                self.state = 'turn'  # 다음 상태를 회전으로 전환
                self.start_theta = self.pose.theta  # 회전 시작 시점 방향 저장
            ...
            if abs(dtheta) < self.angle:
                msg.angular.z = angular_velocity  # 아직 90도 못 돌았으면 회전 계속
            else:
                msg.angular.z = 0.0  # 회전 완료 시 정지
                self.turn_count += 1  # 꼭짓점 하나 완료
                if self.turn_count >= 4:
                    self.turn_count = 0       # 꼭짓점 카운터 초기화
                self.state = 'forward'        # 다시 직선으로 돌아감 → 반복 시작
